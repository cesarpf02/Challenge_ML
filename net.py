...
class Net(nn.Module):
    def __init__(self, use_dropout=True, use_bn=None, channel_input=1):
        ...
        # Convs layers
        self.conv1 = nn.Conv2d(channel_input, 32, 5)   # input is channel_input image, 32 output channels/feature maps, 5x5 kernel / window
        self.conv2 = nn.Conv2d(32, 64, 5)  # input is 32 as the first layer outputs 32 channels/feature maps. At the currente layer we apply 64 channels with 5x5 kernel-window
        ...
        x = torch.randn(channel_input, 64, 4096).view(-1, channel_input, 64, 4096) # create random data
    ...
    def convs(self, x):
        ...
        # The input is already nomralized! Batch norm skipped the first layer!
        x = self.conv1(x)
        if self.use_bn:
            x = self.bn1(x)
        x = F.relu(x)
