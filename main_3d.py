...
train_X=train_images_tensor.view(-1,1,64,64,64)
...
test_X=test_images_tensor.view(-1,1,64,64,64)
print('testX',test_X.shape)
print('testY',test_y.shape)
# Define the model
net_3d = net_3d.Net(use_dropout=USE_DROPOUT, use_bn=USE_BN, channel_input=CHANNEL_SIZE)
net_3d.to(device)
print(net_3d)

# Define the loss and optimizer
optimizer = optim.Adam(net_3d.parameters(), lr=LR) # all the net parameters are controlled by the defined optimiser
...
for epoch in range(EPOCHS):
    ...
    for phase in ['train', 'val']:
        if phase == 'train':
            net_3d.train()  # Set model to training mode
            data_len = len(train_X)
        else:
            net_3d.eval()  # Set model to evaluate mode
            data_len = len(test_X)
        ...
        for i in tqdm(range(0, data_len, BATCH_SIZE)):  # from 0, to the len of x, stepping BATCH_SIZE at a time.
            if phase == 'train':
                batch_X = train_X[i : i + BATCH_SIZE].view(-1, 1, 64, 64,64)
                print('batchX', batch_X.shape)
                batch_y = train_y[i : i + BATCH_SIZE]
                print('batchY', batch_y.shape)

            elif phase == 'val':
                batch_X = test_X[i: i + BATCH_SIZE].view(-1, 1, 64, 64,64)
            ...
            with torch.set_grad_enabled(phase == 'train'):
                outputs = net_3d(batch_X)  # compute the output for a batch of data - forward
                print(outputs.shape)
                print(batch_y.shape)
...
correct = 0
total = 0
net_3d.eval()  # import if there are dropout layers in the model (that are set to OFF in testing phase)
with torch.no_grad():
    for i in tqdm(range(len(test_X))):
        real_class = torch.argmax(test_y[i])
        inputs = test_X[i].view(-1, CHANNEL_SIZE, 64, 64, 64)
        net_out = net_3d(inputs)[0]  # returns a list
